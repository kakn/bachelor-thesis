...
def training(training_model, encoder_input_data, decoder_input_data, decoder_target_data, epochs, batch_size):
    ...
    nowtrain = True
    csv_logger = CSVLogger("2combo_model_history_log.csv", append=True)
    ...
    training_model.save_weights('2combo.h5')
...
def modellen(training_model, decoder_inputs, decoder_lstm):
    ...
    if nowtrain == False:
        training_model.load_weights('2combo.h5')
        print("working!")
...
def testra():
    ...
    for genre in genres:
        ...
        i += 1
    np.savetxt('2b-bleuscores.csv', data, delimiter=',')
